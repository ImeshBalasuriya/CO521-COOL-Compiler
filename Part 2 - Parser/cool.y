...
%{
  ...
  for the location of tokens */
    ...
    */
    ...
    /* Expressions */
    expression:
	OBJECTID ASSIGN expression	/* Assign statements */
	{ $$ = assign($1, $3); }

	| expression '@' TYPEID '.' OBJECTID '(' expr_list ')'	/* Static Dispatch */
	{ $$ = static_dispatch($1, $3, $5, $7); }

	| expression '.' OBJECTID '(' expr_list ')'			/* Non-static Dispatch */
	{ $$ = dispatch($1, $3, $5); }

	| OBJECTID '(' expr_list ')'				/* Self Dispatch */
	{ $$ = dispatch(object(idtable.add_string("self")), $1, $3); }

	| IF expression THEN expression ELSE expression FI	/* Conditionals */
	{ $$ = cond($2, $4, $6); }

	| WHILE expression LOOP expression POOL	/* Loops */
	{ $$ = loop($2, $4); }

	| '{' block_expr '}'	/* Block Expressions */
	{ $$ = block($2); }

	| LET let_stmt		/* Let Statements */
	{ $$ = $2; }

	| CASE expression OF case_list ESAC	/* Case Statements */
	{ $$ = typcase($2, $4); }

	| NEW TYPEID		/* New Expressions */
	{ $$ = new_($2); }

	| ISVOID expression		/* isvoid Expressions */
	{ $$ = isvoid($2); }
	...
	| '(' expression ')'		/* Bracketed Expressions */
	{ $$ = $2; }
	...
	| expr_list ',' expression	/* Multiple Expressions */
